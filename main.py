...
class ImageProcess(object):
    ...
    def get_algorithm(self):
        ...
        for algorithm in self.option_dict["algorithm"]:
            ...
            if algorithm == "DAISY":
                algorithms.append(my_daisy.DAISY())
            if algorithm == "STRUCTURE":
                algorithms.append(my_structure.STRUCTURE())
            if algorithm == "EIGVALS":
                algorithms.append(my_eigval.EIGVALS())
            if algorithm == "CORNER":
                algorithms.append(my_corner.CORNER())
            if algorithm == "KITCHEN":
                algorithms.append(my_kitchen.KITCHEN())
            if algorithm == "TRIZ":
                algorithms.append(my_triz.TRIZ())
        return algorithms
    ...
    def merge_dataset(self, i):
        ...
        if self.option_dict["njob"] == 1:
                ...
                if dataset_index == 0:
                    left, name_list = self.image_read(algorithm_list[i])
    ...
    def save_dataset(self, dataset, i):
        ...
        algorithm = self.option_dict["algorithm"][i]
        ...
        df=pd.read_csv("features/{}_{}.csv".format(saving_name, algorithm), keep_default_na=False)
        my_list = ['a']
        for i in range(1,df.shape[1]):
            my_list.append( algorithm + str(i))
        df.columns=my_list
        df.to_csv("features/{}_{}.csv".format(saving_name, algorithm),index = 0)
        print("\nsuccessful!\n")
